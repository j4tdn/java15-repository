create database shopping_recreate char set utf8mb4;
...
select * from shopping_recreate.nhanvien;
delete from shopping_recreate.nhanvien where MaNV = 'Văn Hoang';
-- 2. Xóa chức vụ giao hàng trong hệ thống 
-- sửa bài 
-- select * from shopping_recreate.chucvu;
-- delete from shopping_recreate.chucvu where TenCV = 'Nhân viên giao hàng';
...
-- select * from shopping_recreate.mathang;
-- select * from shopping_recreate.loaihang;
-- delete from shopping_recreate.mathang where MaLH in ( select MaLH from loaihang where TenLH = 'Mũ');
  delete from mathang
  where MaLH in ( select MaLH from loaihang where TenLH = 'Mũ');
  select * from mathang where MaLH = 5;
-- 4. Xóa tất cả các mặt hàng trong hệ thống
select * from shopping_recreate.mathang;
delete from shopping_recreate.mathang;
-- 5. Vì hệ thống bị lỗi. Hủy tất cả các đơn hàng ngày 23/11/2019
-- select * from shopping_recreate.donhang;
-- delete from shopping_recreate.donhang where ThoiGianDatHang like '2021-01-05%'; -- convert datetime into varchar
-- Hủy khi có 1 đơn hàng và hủy
-- Xóa luôn đơn hàng không cần thiết
select * from donhang where cast(ThoiGianDatHang as Date) = str_to_date('18/12/2020','%d/%m/%y');
-- '2021-12-18' default format y-m-d
-- kiểu time_stamp : dạng date time
select cast(current_timestamp() as date );
-- hàm str_to_date('') : tùy chỉnh format theo yêu cầu người dùng
